...
int InsertElementAt(LinkedList* Liste, int i, Enregistrement pers) {
	...
	if (CurrentElement != NULL) {
		...
		if (Liste->size <= i) { // insertion en queue
			NewElement = NewLinkedListElement(pers);
			if (NewElement != NULL) {
				Liste->tail->next = NewElement;	/*On fait pointer la queue vers l'élément ajouté*/
				Liste->tail = NewElement;		/*On a donc une nouvelle queue*/
				Liste->size++;
				return(1);
			}
			else {
...
int DeleteLinkedListElem(LinkedList* list, SingleLinkedListElem* item) {
	...
	int CurrentIndex = 0;
	SingleLinkedListElem* CurrentElement = list->head;
	while ((CurrentIndex < list->size) && (CurrentElement != item)) {
		CurrentIndex++;
		CurrentElement = CurrentElement->next;
	}
	if (CurrentIndex < list->size) {
		GetElementAt(list, CurrentIndex - 1)->next = CurrentElement->next;
		free(CurrentElement);
		return(1);
	}
	return(0);  // pas trouvé
